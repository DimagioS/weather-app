<template>
  <div class="card-box">
    <div class="container">
      ...
      <app-weather-card
        @open-details="$emit('to-details', $event)"
        @remove-card="$emit('remove-card', $event)"
        :dark-theme='darkTheme'
        :main-card-data="mainCardData"
      ></app-weather-card>

      <div class="add__card" :class="{'add__card-dark': darkTheme}">
        <div class="header__container">
          <span class="card__title">Добавить город</span>
        </div>
        <div class="body__container">
          <router-link to="/search" @click="toSearch">
            <svg v-if="!darkTheme" class="add__icon" id="Plus_Icon" data-name="Plus Icon" viewBox="1454.4 326.4 209.1 209.1">
              <defs>
            ...
            <svg v-if="darkTheme" class="add__icon" viewBox="0 0 209.1 209.1">
              <defs>
              ...
              <g data-name="Plus Icon" transform="translate(-1454.4 -326.4)">
                ...
                <rect width="12.8" height="142.9" fill="url(#b)" class="cls-300" data-name="Rectangle 3" rx="5"
                      transform="translate(1552.6 358.1)"/>
                <rect width="12.8" height="142.9" fill="url(#c)" class="cls-300" data-name="Rectangle 4" rx="5"
                      transform="rotate(90 603.6 1026.8)"/>
              </g>
            </svg>
          </router-link>
          <svg v-if="!darkTheme" class="city__illustration" id="City_Illustration" data-name="City Illustration"
          ...
          <svg v-if="darkTheme" class="city__illustration" viewBox="0 0 405.6 331.4">
            <defs>
...
<script>
  ...
  export default {
    emits: {
      'to-details': $event => {
        if ($event) return true
          ...
          console.warn('Некорректные данные для генерации события "to-details"!');
          return false
        } 
      },
      'remove-card': $event => {
        if ($event) return true
          ...
          console.warn('Некорректные данные для генерации события "remove-card"!');
          return false
        } 
      }
    },
    ...
    methods: {
      toSearch() {
        setTimeout(() => document.querySelector('.root__container').querySelector('.search-city-input').focus(), 0)
      }      
    },
...
<style lang='scss' src="../style/add-card.scss" scoped></style>